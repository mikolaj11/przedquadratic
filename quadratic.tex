...
groups in terms certain quantities $(C_i)$. 
%which describe the macroscopic shape of the 
%Young diagram. 
Goulden--Rattan positivity 
...
\item It has one face and one 
edge (\emph{the root}) is distinguished.

\item To each black vertex is assigned a weight, 
which is a natural number. We assume that each 
...
$V$ denotes the number of the vertices and 
$k$ denotes the number of the edges.
...
Each special edge has two ends. Some 
ends of the special edges have assigned 
one of two directions clockwise or 
counterclockwise.
...
\caption
{
	...
	\protect\subref{fig:rysprzed} 
		...
		dashed and coloured red.
	\protect\subref{fig:rysw} 
		The graph during the edge 
		sliding. Both end of each 
		special edge has assigned 
		direction clockwise.
...
of maps. Any such a map has one face 
...
Of course $k_1+k_2=\degg(b_1)$ is 
an odd number. Without loss of the 
...
\caption
{
The examples of the maps from the sets 
...
The directions and the root are not marked.
}
\label{fig:rysrest}
\end{figure}
...
Let $\sigma$ be the cycle that 
encodes the unique face of $G$. 
(like for the edge sliding with empty 
set of the special edges) We will say 
that \emph{the vertex $w_j$ is the 
\successor of the vertex $w_i$} 
(we denote it by $w_i \rightarrow w_j$) 
if using the clockwise cyclic order of 
...
Any such a map has one face and an odd 
...
are colored in three colors: $1, 2, 3$
(according to the names of the vertices 
they are in). If a corner $c$ has the 
color $a$, its \successor $\nast^2(c)$ 
has either the color $a$ or 
$1+a \operatorname{mod} 3$. 
There is only one corner which has the 
color $1$, so the corner colors 
...
becouse exists a unique corner $c_0=\nast(c_2)$
...
contradiction, because such maps does not 
exist (see \cref{euler}). Therefore, the
map $m$ has white vertex with an odd degree 
greater than $1$.
\end{proof}
...
of the special edges we select all edges of
the vertex $b_1$. The ends of the special edges 
at the vertex $b_1$ have no assigned the direction 
and the other ends have assigned
the direction counterclockwise. We apply the 
edge sliding on the map $m$. Then we change 
the color of the black vertex $b_1$ to white 
...
becouse any map from the set $X_{k}$ has at 
least one edge between each pair of the vertices 
of different colours. We obdained the map from the 
set $T_{k}^{\odd}$. Moreover, each map from the 
set $T_{k}^{\odd}$ can be produced. Such 
...
We start from a map $m \in X_{k}$. As the set
of the special edges we select all edges of
the vertex $b_2$. The ends of the special edges 
at the vertex $b_2$ have no assigned the 
direction and the other ends have assigned
the direction counterclockwise. We apply the 
edge sliding on the map $m$. Then we change 
the color of the black vertex $b_2$ to white 
and its name to $w_3$, and the name of the vertex
...
least one edge between each pair of the vertices 
of different colours. We obdained the map from the 
set $T_{k}^{\even}$. Moreover, each map from the 
set $T_{k}^{\even}$ can be produced. Such 
transformation is a bijection between the set 
...
the vertex $b_1$. The ends of the special edges 
at the vertex $b_1$ have no assigned the direction 
and the ends at the vertex $w_1$ have assigned
...
becouse any map from the set $X_{k}$ has at 
least one edge between each pair of the vertices 
of different colours. It can be that we not 
obdained the map from the set $T_{k}^{\odd}$, 
but each map from the 
set $T_{k}^{\odd}$ can be produced. Such 
transformation is a bijection between the set 
$X_k$ and a some superset of the set $T_k^{\odd}$.
